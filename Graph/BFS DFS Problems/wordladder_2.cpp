...
//             n++;
//         }
//         // for each word, we're making adjency list of neighbour words (words
//         // that can be made with one letter change) Paths will store all the
//         // shortest paths (formed later by backtracking)
//         vector<vector<int> > g(n, vector<int>()), Paths;

//         // storing possible parents for each word (to backtrack later), path is
//         // the current sequence (while backtracking)
//         vector<int> parent[n], path;

//         // creating adjency list for each pair of words in the wordList
//         // (including beginword)
//         for (int i = 0; i < n - 1; i++)
//             for (int j = i + 1; j < n; j++)
